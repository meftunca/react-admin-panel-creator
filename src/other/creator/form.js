...
class FormCreator extends Component {
  ...
  handleClick = () => {
    ...
    let column = [];
    if (values["formItem"] == undefined || values["formItem"].length == 0)
      return alert("Lütfen input oluşturun");
  ...
  remove = i =>
    this.setState({ appendInput: this.state.appendInput.filter(k => k != i) });
  render() {
...
const PreviewDialog = ({ open, toggle, data }) => (
  <Dialog
    ...
    open={open}
    onClose={toggle}
    aria-labelledby="form-preview-dialog"
  >
    <DialogContent>
      <br />
      <br />
  ...
  render() {
    ...
    return (
      <BottomNavigation
        ...
        showLabels
        className={classes.bottomManagement}
      >
        <BottomNavigationAction
          onClick={send}
          label="Gönder ve yükle"
          icon={<i className="material-icons">send</i>}
        />
        <BottomNavigationAction
          onClick={view}
          label="Göster"
          icon={<i className="material-icons">rate_review</i>}
        />
        <BottomNavigationAction
          onClick={add}
