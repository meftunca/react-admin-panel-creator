            ...
            <Fragment>
                <Grid container className={classes.root} spacing={2}>
                    <Grid item xs={12} md={12}>
                        <MailHeader classes={classes} refs={this.mailSearchHeader} reload={this.onReload} search={this.handleQuerySearch} />
                    </Grid>

                    <Grid item sm={12} md={4} lg={3} xl={2}>
                        <Button color='secondary' className={classes.button} onClick={this.openEditor}>
                            <Icon className={classes.extendedIcon}>add</Icon> Yeni Mail
                        </Button>
                        <List>
                    ...
                    <Grid item sm={12} md={8} lg={9} xl={10}>
                        ...
                        <List className={classes.rootList}>
                            {mailPagList != [] &&
                                mailPagList.map((i, k) => (
                                    <MailContent key={uniqid()} classes={classes} id={i} remove={this.removeMail} updateSendData={this.updateSendData} />
                                ))}
                        </List>
                    </Grid>
                </Grid>

                <FullScreenDialog open={this.state.modal} handleClose={this.handleClose} save={this.onSave} send={this.sendEmail}>
                    <div className={classes.modalWrapper}>
                        <Grid container>
                            <Grid item xl={12} md={12}>
                                <Grid container direction='row' spacing={3}>
                                    <Grid item md={6} sm={12}>
                                        <TextField id='outlined-to' autoComplete='email' inputRef={this.to} label='Alıcı' margin='normal' fullWidth />
                                    </Grid>
                                    <Grid item md={6} sm={12}>
                                        <TextField fullWidth id='outlined-subject' inputRef={this.subject} label='Konu' margin='normal' />
                                    </Grid>
                                </Grid>
                            </Grid>
                            <br />
...
function MailHeader({ classes, refs, reload, search }) {
    return (
        <Paper className={classes.rootHeader} elevation={2}>
        ...
        <Fragment>
            ...
            <Collapse in={open} timeout='auto' unmountOnExit>
                <div className={classes.mailBoxCollapse}>
                    <Typography variant='body2' gutterBottom className={classes.extendedIcon} dangerouslySetInnerHTML={{ __html: data.textPlain }} />
                    <Typography variant='body2' gutterBottom className={classes.extendedIcon} dangerouslySetInnerHTML={{ __html: data.textHtml }} />
                </div>
            </Collapse>
        </Fragment>
    )
