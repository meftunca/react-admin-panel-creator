...
class TableBuilder extends Component {
    ...
    getData = async () => {
        ...
        axios.post("/" + name + "/find", { options: [{}] }).then(({ data }) => {
            ...
            this.setState({ column: columns, data: dataArr, originalData: data, show: true })
        })
    ...
    delete = (list) => {
        ...
        Object.keys(list.lookup).map((i) => {
            axios
                .post("/" + name + "/remove", { options: [this.state.data[Number(i)]._id] })
                ...
                return (
                    <div>
                        <Button
                            ...
                            component={(itemProps) => (
                                <Link to={"/forms/edit" + route.path + "/" + originalData[displayData[0].dataIndex]._id} {...itemProps} />
                            )}>
                            <Icon className={classes.leftIcon}>edit</Icon>
                            Düzenle
                            {/* This Button uses a Font Icon, see the installation instructions in the docs. */}
                        </Button>
                        <Button color='secondary' className={classes.button} onClick={() => this.delete(selectedRows)}>
